...
int main(void) {
    ...
    */
    ...
    for (unsigned int i=0; i<matches; i++) {
        bool is_match = i!=0 ? domino_matches(pieces[i-1],pieces[i]) : true;

        if (!is_match) // si no son iguales doy vuelta p2
        {
            domino_dump(domino_flip(pieces[i]));
        } else {domino_dump(pieces[i]);}
    }
    // Free memory
